...
def create_u_vector(L, num_subintervals_lattice):
    ...
    u_vector = np.zeros(num_subintervals_lattice + 1)  # Add 1 for right endpoint of lattice
...
def calculate_I(gamma_matrix, lattice_points, selectivity_range, n_vec, spatial_step):
    ...
    for i in range(len(lattice_points)):
        ...
        binding_vector[i] = np.sum(spatial_step * np.multiply.reduce([g_vec, n_vec, gamma_matrix[i, :]]))
...
def update_parameters(nC_k, nT_k, time_step, spatial_step, R_c, R_t, beta_c, central_difference_matrix):
    '''
    ...
    '''
    # Cancer cells
    nC_k_plus_one_half = np.multiply(nC_k, np.divide(1 + time_step * np.maximum(0, R_c), 1 + time_step * abs(np.minimum(0, R_c))))
    nC_k_plus_one = np.zeros(len(nC_k))
    nC_k_plus_one[1:-1] = nC_k_plus_one_half[1:-1] + beta_c * (time_step / spatial_step ** 2) * central_difference_matrix.dot(nC_k_plus_one_half)[1:-1]
    ...
    # T-cells
    nT_k_plus_one = np.multiply(nT_k, np.divide(1 + time_step * np.maximum(0, R_t), 1 + time_step * abs(np.minimum(0, R_t))))
...
def interaction_matrix_model(L, num_subintervals_lattice, time_step, final_time, a, A, theta_c, theta_t, eta, alpha_c, mu_c,
                             zeta_c, alpha_t, mu_t, zeta_t, gamma_matrix, lambda_c, plot_cell_evolution=True,
                             print_results=True):
    '''
    ...
    while time <= final_time:
        ...
        gamma_Jt = calculate_I(gamma_matrix.T, v_vector, eta, nC_k, spatial_step) # Input transpose of gamma_matrix so that rows show the binding affinity of each T-cell
        ...
        # Update vectors
        nC_k, nT_k = update_parameters(nC_k, nT_k, time_step, spatial_step, R_c, R_t, beta_c, matrix)
        nC_matrix.append(nC_k)
        nT_matrix.append(nT_k)
...
def matrix_exponential_dist(num_subintervals_lattice, delta):
    ...
    '''
    matrix = np.random.exponential(scale=delta, size=(num_subintervals_lattice + 1, num_subintervals_lattice + 1))
    return matrix
...
if __name__ == '__main__':
    ...
    nC, nT, u_vector, time_vector = interaction_matrix_model(L=1, num_subintervals_lattice=100, time_step=0.05,
                                  final_time=100, a=1, A=5, theta_c=1.8, theta_t=1.8, eta=2, alpha_c=1.5,
                                  mu_c=5 * 10 ** (-6), zeta_c=5 * 10 ** (-6), alpha_t=0.05, mu_t=5 * 10 ** (-6),
                                  zeta_t=3 * 10 ** (-5), gamma_matrix=gamma_mat, lambda_c=0.01)
